...
static void app_task_1(void *pvParameters)
{
	...
	for(;;){
		...
		if(strstr((char *)g_usart_packet.rx_buf,"led")){
			if(strstr((char *)g_usart_packet.rx_buf,"on")){
				led_ctrl(0,1);
			}
			else if(strstr((char *)g_usart_packet.rx_buf,"off")){
				led_ctrl(0,0);
			}
		}
		else if(strstr((char *)g_usart_packet.rx_buf,"sr04")){
		...
		else if(strstr((char *)g_usart_packet.rx_buf,"dht11")){
