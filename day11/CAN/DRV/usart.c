		...
		if(usart1_flag){
			...
			if(strstr((char *)usart1_buf,"dht11")){
				if(dht11_read_data(&temp,&humi)==0){
					...
					//flash_write(temp,humi);
					if(temp>27){
						RTC_GetTime(RTC_Format_BCD,&RTC_TimeStruct);
						sprintf((char *)usart1_buf,"%hhx:%hhx:%hhx %d℃",
								RTC_TimeStruct.RTC_Hours,RTC_TimeStruct.RTC_Minutes,RTC_TimeStruct.RTC_Seconds,temp);
						w25q128_erase_sector(0x0);
						w25q128_write_page(0x0,(u8 *)usart1_buf,16);
					}
				}
				else{
			...
			//读取flash温湿度
			if(strstr((char *)usart1_buf,"getflash")){
				temp = *(u8 *)SECTOR_5_ADDRESS;
				humi = *(u8 *)(SECTOR_5_ADDRESS+1);
				printf("temp = %d,humi = %d\r\n",temp,humi);
			}
			
			//读取flash温湿度
			if(strstr((char *)usart1_buf,"getspi")){
				w25q128_read_data(0x0,(u8 *)usart1_buf,16);
				printf("%s\r\n",(char *)usart1_buf);
			}
			...
			//设置时间
			if(strstr((char *)usart1_buf,"settime")){
				char *buf = NULL;
				u8 hour,min,sec;
				
				buf = strtok((char *)usart1_buf,":");
				if(buf==NULL){
					usart3_puts("settime commad error!");
				}
				else{
					buf = strtok(NULL,":");
					if(buf==NULL){
						usart3_puts("settime hour error!");
					}
					else{
						hour = atoi(buf);
						...
						if(buf==NULL){
							...
							if(buf==NULL){
								...
								if(hour<24&&min<60&&sec<60){
									...
									usart3_puts("settime successed!");
								}
								else{
									usart3_puts("hour min sec error!");
								}
							}
						}
					}
