...
//		}
		...
		//从消息队列接收消息
//		xRes = xQueueReceive(g_queue_usart,&usart_packet,portMAX_DELAY);
//		if(xRes!=pdPASS){
//			printf("xQueueReceive failed,error code = %d!\r\n",xRes);
//			continue;
//		}
		
		//等待事件标志组 ----- 等待串口发消息 和 定时器超时
		xEventGroupWaitBits(g_event_group,
							0x3,//等待低2位
							pdTRUE,//返回后清0
							pdTRUE,//与同步,低两位同时为1
							//pdFALSE,//或同步,低两位有1位为1
							portMAX_DELAY);
...
//		printf("command = %s\r\n",usart_packet.rx_buf);
//		
//		if(strstr((char *)usart_packet.rx_buf,"led")){
//			if(strstr((char *)usart_packet.rx_buf,"on")){
//				led_ctrl(0,1);
//			}
//			else if(strstr((char *)usart_packet.rx_buf,"off")){
//				led_ctrl(0,0);
//			}
//		}
//		else if(strstr((char *)usart_packet.rx_buf,"sr04")){
//			distance = get_distance();
//			if(distance==-1){
//				printf("get distance error!\r\n");
//			}
...
//			}
//		}
//		else if(strstr((char *)usart_packet.rx_buf,"dht11")){
//			if(dht11_read_data(buf)==0){
//				printf("temp = %d,humi = %d!\r\n",buf[2],buf[0]);
//				sprintf((char *)temp_buf,"temp = %d,humi = %d",buf[2],buf[0]);
//				OLED_ShowString(0,2,temp_buf,17);
//			}
//			else{
...
static void soft_timer_callback(TimerHandle_t pxTimer)
{
	//printf("soft_timer_callback running!\r\n");
	PFout(9) = ~PFout(9);
	
	//printf("vol = %d\r\n",adc_voltage_get());
	
	//设置事件标志
	xEventGroupSetBits(g_event_group,0x1);//最低位置1
}
