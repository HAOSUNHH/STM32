...
volatile struct usart_packet_t g_usart_packet={0};
...
extern SemaphoreHandle_t g_sem_1;
...
static 
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  
	{
		...
		if(d=='#' || i>=(sizeof g_usart_packet.rx_buf))
		{
			...
			//设置事件标志
			xEventGroupSetBitsFromISR(g_event_group,0x2,&xHigherPriorityTaskWoken);//第2位置1
			...
			//通知任务处理信息
			//xSemaphoreGiveFromISR(g_sem_1,&xHigherPriorityTaskWoken);
