...
#define CPU_FREQUENCY_MHZ    100		// STM32时钟主频，对应你自己的MCU主频改一下
...
void Touch_Test(void)
{
    ...
    //这里是一个按键，我这里是PA0脚，按下这个键就会重新校正屏幕 		
		if(HAL_GPIO_ReadPin(Touch_Adjust_Key_GPIO_Port , Touch_Adjust_Key_Pin) == 0)	//KEY_RIGHT按下,则执行校准程序
		{
			LCD_Clear(WHITE);//清屏
		  TP_Adjust();  //屏幕校准  
			LCD_Clear(WHITE);
